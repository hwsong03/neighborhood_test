Shader "Unreal/M_BASE_MASTER"
{
	Properties
	{
		_MainTex("MainTex (RGB)", 2D) = "white" {}
		Material_Texture2D_0( "Normal_Tex", 2D ) = "white" {}
		Material_Texture2D_1( "BC_Tex", 2D ) = "white" {}
		Material_Texture2D_2( "Grunge_Tex", 2D ) = "white" {}
		Material_Texture2D_3( "ORM_Tex", 2D ) = "white" {}
	...
	SubShader 
	{
		...
		//BLEND_ADDITIVE Tags { "RenderType" = "Transparent"  "Queue" = "Transparent" }
		
		//#pragma handles transparency states
		Cull Off
		...
		//BLEND_ADDITIVE #pragma surface surf Standard vertex:vert alpha:additive addshadow
		...
		// Zone 변수들 (Local + Remote 통합)
		uniform int _ZoneMode; // 0=Off, 1=Local, 2=Remote
		uniform float _P;
		uniform float _ZoneRadius;
		uniform float _EnableZoneClipping;
		uniform int _Length;
		uniform float4 _Users[10];
		uniform fixed4 _Colors[10];
		uniform int _WhichRegion;
		uniform int _BaseRegion;
		
		// RemoteZone 전용 (필요시)
		uniform float4 _EachZone[1024];
		uniform int _EachZoneCount;
		...
		// 통합된 Zone 계산 함수
		void calculateZoneLogic(float3 worldPos, out int closestZone, out bool shouldClip, out bool shouldRender)
		{
			float minDist = 1e8;
			closestZone = 0;
			shouldClip = false;
			...
			// Zone Mode가 Off면 바로 리턴
			if (_ZoneMode == 0)
			{
				return;
			}
			
			// 1. 가장 가까운 Voronoi 영역 찾기
			...
			// 3. Zone Mode에 따른 로직 분기
			if (_ZoneMode == 1) // LocalZone
			{
				// LocalZone: 다른 사용자 원형 영역에서 숨김
				shouldClip = isInAnyCircle;
				shouldRender = !shouldClip;
			}
			else if (_ZoneMode == 2) // RemoteZone
			{
				// RemoteZone: 다른 사용자 원형 영역에서만 보여주고, 자신의 영역에서만 렌더링
				if (!isInAnyCircle)
				{
					shouldClip = true;
					shouldRender = false;
				...
				{
					// 자신의 영역(_WhichRegion)인지 확인
					if (closestZone == _WhichRegion)
					{
						shouldClip = false;
						shouldRender = true;
					}
...
void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
	...
	MaterialFloat3 Local10 = (((MaterialFloat3)1.00000000) - Local8.rgb);
	MaterialFloat3 Local11 = (Local10 * ((MaterialFloat3)2.00000000));
	MaterialFloat3 Local12 = (Local11 * Material.PreshaderBuffer[6].xyz);
	MaterialFloat3 Local13 = (((MaterialFloat3)1.00000000) - Local12);
	MaterialFloat3 Local14 = (Local8.rgb * ((MaterialFloat3)2.00000000));
	MaterialFloat3 Local15 = (Local14 * Material.PreshaderBuffer[5].xyz);
	MaterialFloat Local16 = select((Local8.rgb.r >= 0.50000000), Local13.r, Local15.r);
	MaterialFloat Local17 = select((Local8.rgb.g >= 0.50000000), Local13.g, Local15.g);
	MaterialFloat Local18 = select((Local8.rgb.b >= 0.50000000), Local13.b, Local15.b);
	MaterialFloat2 Local19 = (DERIV_BASE_VALUE(Local0) * ((MaterialFloat2)Material.PreshaderBuffer[8].x));
	MaterialFloat Local20 = MaterialStoreTexCoordScale(Parameters, DERIV_BASE_VALUE(Local19), 5);
	MaterialFloat4 Local21 = ProcessMaterialColorTextureLookup(Texture2DSampleBias(Material_Texture2D_2,Material_Texture2D_2Sampler,DERIV_BASE_VALUE(Local19),View.MaterialTextureMipBias));
	MaterialFloat Local22 = MaterialStoreTexSample(Parameters, Local21, 5);
	MaterialFloat3 Local23 = lerp(MaterialFloat3(MaterialFloat2(Local16,Local17),Local18),Material.PreshaderBuffer[8].yzw,Local21.r);
	MaterialFloat Local24 = MaterialStoreTexCoordScale(Parameters, DERIV_BASE_VALUE(Local1), 0);
	MaterialFloat4 Local25 = ProcessMaterialColorTextureLookup(Texture2DSampleBias(Material_Texture2D_3,Material_Texture2D_3Sampler,DERIV_BASE_VALUE(Local1),View.MaterialTextureMipBias));
	MaterialFloat Local26 = MaterialStoreTexSample(Parameters, Local25, 0);
	MaterialFloat Local27 = (Local25.g + Material.PreshaderBuffer[9].x);
	MaterialFloat Local28 = (Local21.r + Material.PreshaderBuffer[9].y);
	MaterialFloat Local29 = (Local27 + Local28);
	...
	Parameters.WorldTangent = 0;
		...
		void surf( Input In, inout SurfaceOutputStandard o )
		{
			...
			bool shouldClip;
			bool shouldRender;
			calculateZoneLogic(In.worldPos, closestZone, shouldClip, shouldRender);
			
			// Zone Mode가 활성화되고 클리핑 조건이면 픽셀 버리기
			if (_ZoneMode > 0 && _EnableZoneClipping > 0.5 && shouldClip)
			{
