...
Shader "AVP/Velvet"
{
	...
	SubShader
	{
		...
		struct Input
		{
			float2 uv_texcoord;
			float3 viewDir;
			float3 worldPos; // Zone 계산을 위해 추가
			INTERNAL_DATA
		};
		...
		bool calculateZoneClip(float3 worldPos)
		{
			...
			else if (_ZoneMode == 2) // RemoteZone
			{
				...
				{
					shouldClip = (closestZone != _WhichRegion);
				}
			}
		...
		{
			// Zone clipping check - 가장 먼저 실행
			if (calculateZoneClip(i.worldPos))
			{
				clip(-1);
				return;
			}
